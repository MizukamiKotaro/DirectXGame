...
Matrix3x3 Matrix3x3::Inverse(const Matrix3x3& m) {
	...
	for (int row = 0; row < 3; row++) {
		for (int column = 0; column < 3; column++) {
			if ((row + column) % 2 == 0) {
				result.m[row][column] = num * (m2[column][row].m[0][0] * m2[column][row].m[1][1] -
				                               m2[column][row].m[0][1] * m2[column][row].m[1][0]);
			} else {
				result.m[row][column] = -num * (m2[column][row].m[0][0] * m2[column][row].m[1][1] -
				                                m2[column][row].m[0][1] * m2[column][row].m[1][0]);
			}
...
Matrix3x3 Matrix3x3::Inverse() { return Inverse(*this); }

Matrix3x3 Matrix3x3::MakeAffineTransformMatrix(
    const Vector2& scale, float radian, const Vector2& translate) {
	Matrix3x3 matrix = {
...
Matrix3x3 Matrix3x3::MakeViewportMatrix(float left, float top, float right, float bottom) {
	Matrix3x3 matrix = {};
	matrix.m[0][0] = (right - left) / 2;
	matrix.m[0][1] = 0;
	matrix.m[0][2] = 0;
	matrix.m[1][0] = 0;
	matrix.m[1][1] = (top - bottom) / 2;
	matrix.m[1][2] = 0;
	matrix.m[2][0] = left + (right - left) / 2;
	matrix.m[2][1] = top + (bottom - top) / 2;
	matrix.m[2][2] = 1;
	return matrix;
}

Matrix3x3 Matrix3x3::MakeWvpVpMatrix(
    const Vector2& objectScale, float objectRadian, const Vector2& objectTranslate,
    const Vector2& cameraScale, float cameraRadian, const Vector2& cameraTranslate,
    const Vector2& orthoTopLeft, const Vector2& orthoBottomRight, const Vector2& viewportTopLeft,
    const Vector2& viewportBottomRight) {

	Matrix3x3 objectWorldMatrix =
	    Matrix3x3::MakeAffineTransformMatrix(objectScale, objectRadian, objectTranslate);
	Matrix3x3 cameraWorldMatrix =
	    Matrix3x3::MakeAffineTransformMatrix(cameraScale, cameraRadian, cameraTranslate);
	Matrix3x3 viewMatrix = Matrix3x3::Inverse(cameraWorldMatrix);
	Matrix3x3 orthoMatrix = Matrix3x3::MakeOrthographicMatrix(
	    orthoTopLeft.x, orthoTopLeft.y, orthoBottomRight.x, orthoBottomRight.y);
	Matrix3x3 ViewportMatrix = Matrix3x3::MakeViewportMatrix(
...
Matrix3x3 operator+(const Matrix3x3 m1, const Matrix3x3& m2) { return Matrix3x3::Add(m1, m2); }

Matrix3x3 operator-(const Matrix3x3 m1, const Matrix3x3& m2) { return Matrix3x3::Subtract(m1, m2); }

Matrix3x3 operator*(const Matrix3x3 m1, const Matrix3x3& m2) { return Matrix3x3::Multiply(m1, m2); }

Matrix3x3 operator*(float s, const Matrix3x3& m) { return Matrix3x3::Multiply(s, m); }

Matrix3x3 operator*(const Matrix3x3& m, float s) { return Matrix3x3::Multiply(s, m); }

Vector2 operator*(const Vector2& v, const Matrix3x3& m) { return Matrix3x3::Multiply(v, m); }

Vector2 operator*(const Matrix3x3& m, const Vector2& v) { return Matrix3x3::Multiply(v, m); }
